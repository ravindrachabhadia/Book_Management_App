...
const EditBookPage = () => {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(false); // For submission loading
  const [isFetching, setIsFetching] = useState<boolean>(true); // For initial data fetching
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      navigate('/'); // Navigate back to the book list on success
    } catch (err: any) {
  ...
  // Show error if initial fetching failed and we don't have a title to display
  ...
  // Render the form once data is fetched (or if fetching failed but we show the form anyway)
  return (
    <Container size="xs" px="xs" style={{ marginTop: '50px' }}>
      <Paper withBorder shadow="md" p={30} mt={30} radius="md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Group justify="flex-end" mt="xl">
             <Button variant="default" onClick={() => navigate('/')} disabled={isLoading || isFetching}>
               Cancel
             </Button>
             <Button type="submit" loading={isLoading} disabled={isFetching}>
               Save Changes
             </Button>
          </Group>
        </form>
